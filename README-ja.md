 ...
 * テーマをダウンロードせずローカルファイルを使う場合に指定してください。ここで指定したファイルを編集すると、仮想マシーンのサイトに反映されます。
 * `/themes` ディレクトリ (e.g. 'twentythirteen') に存在するテーマ名を使用してください。
 * 注: このオプションを設定すると`WP_DEFAULT_THEME` の値が上書きされます。
